import { TOKEN, SERVER_ID, MY_ID, AMY_ID, GLOO_ID, LYKOPHOS_ID, NAMIRE_ID, TEEMOTHEE_ID, QATARI_ID, MODROLE_ID, LOLAPAZ_ID, HATERS_ID, CASPIAN_ID, MURKY_ID, ENEMY_ID } from "./config"; 
...
client.on("messageCreate", async (message) => {
    ...
    if (message.content.toLowerCase().includes("demote")) {
        ...
        if (message.content.toLowerCase().includes("gloo")) {
            ...
            id = TEEMOTHEE_ID!;
        } else if (message.content.toLocaleLowerCase().includes("hate")) {
            id = HATERS_ID;
